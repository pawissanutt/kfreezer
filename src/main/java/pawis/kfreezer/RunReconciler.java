...
@ApplicationScoped
public class RunReconciler implements Reconciler<KFRun>,
        Cleaner<KFRun>{
    ...
    public UpdateControl<KFRun> reconcile(KFRun kfRun, Context<KFRun> context) throws Exception {
        ...
        if (snapshot == null) {
            ...
            LOGGER.info("KFRun '{}': cannot creating a new job because the snapshot '{}' is not completed yet.",
                    kfRun.getMetadata().getName(),
    ...
    private UpdateControl<KFRun> createJob(KFRun run,
                                           KFSnapshot snapshot) {
        LOGGER.info("Creating a job from KFRun '{}' and KFSnapshot '{}'",
                run.getMetadata().getName(), snapshot.getMetadata().getName());
        JobSpec jobSpec = Json.decodeValue(snapshot.getStatus().getJobSpecs(),
                JobSpec.class);
        var labels = jobSpec.getTemplate()
                .getMetadata()
                .getLabels();
        ...
        var initContainers = podSpec
                .getInitContainers();
        var snapshotUrl = snapshotRepo.getUrl(snapshot);

        Volume volume = new VolumeBuilder()
                .withName("snapshot-image")
                .withNewEmptyDir()
                .endEmptyDir()
                .build();
        VolumeMount volumeMount = new VolumeMountBuilder()
                .withMountPath(snapshotPath)
                .withName("snapshot-image")
                .build();
        var con = new ContainerBuilder()
                .withName("snapshot-loader")
                .withImage("rancher/curl")
                .withCommand("sh", "-c")
                .withArgs("""
                        curl -o "%s/image.tar" "%s";
                        tar -xvzf "%s/image.tar" -C "%s"; 
                        """.formatted(snapshotPath,snapshotUrl,snapshotPath,snapshotPath))
        ...
        Job job = new JobBuilder()
                .withNewMetadata()
                .withName(run.getSpec().getJobName())
